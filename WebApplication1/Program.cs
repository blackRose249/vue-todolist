...
{
    containerBuilder.RegisterType<ConstructorInjectionService>().As<IConstructorInjectionService>().InstancePerDependency().EnableInterfaceInterceptors().InterceptedBy(typeof(MyInterceptor));
    containerBuilder.RegisterType<MethodInjectionService>().As<IMethodInjectionService>().InstancePerDependency().EnableInterfaceInterceptors().InterceptedBy(typeof(MyInterceptor));
    containerBuilder.RegisterType<PropertyInjectionService>().As<IPropertyInjectionService>().InstancePerDependency().EnableInterfaceInterceptors().InterceptedBy(typeof(MyInterceptor));
    ...
    // 3.InstancePerLifetimeScope() 作用域生命周期:?个?求或?个操作?元的生命周期中
    ...
    containerBuilder.RegisterType<TestService>().As<ITransientTestService>().InstancePerDependency();   // Transient
    containerBuilder.RegisterType<TestService>().As<IScopedTestService>().InstancePerLifetimeScope();  // Scoped
    containerBuilder.RegisterType<TestService>().As<ISingletonTestService>().SingleInstance();  // Singleton
